...
本项目在**开发与验证阶段**做过一类工作：把模拟器的结算结果与**真实客户端**的行为做对照，
用来发现「我们的模型哪里算错了」。这一节说明这类工作**做了什么、没做什么、产物在哪**。

### 8.1 做了什么：全部是只读观察

| 动作 | 手段（公开手段，硬件/系统自带） | 产物入库？ |
| --- | --- | --- |
| 读取运行中客户端的进程内存（**主要取数手段**） | 操作系统公开的**只读**进程内存接口；**只申请读权限与查询信息权限** —— **无写、无注入、无挂钩、无线程操作** | **否** |
| 确认每次读数出自哪个进程 | 公开的进程枚举接口 ＋ 对客户端可执行文件取 SHA-256 与文件大小；每次读数都附进程身份与启动时刻 | **否** |
| 字段清单与布局对照 | 解析**随客户端安装自带**的元数据文件（**离线**解析；不联网、不改动客户端文件） | **否** |
| 独立第二来源（屏幕） | 对客户端窗口做**外部**截图（**不进进程**） | **否** |
| 屏幕文字与数字的读取 | 公开的多模态视觉接口（第三方服务，经本机代理） | **否** |
| 两侧时间对齐 | 系统**性能计数器**（跨进程同一时钟）作对齐键；两侧各记一次「墙钟 − 性能计数器」，差 > 1 s 判该轮作废 | **否** |

### 8.2 明确**没有**做的事

* **没有**尝试绕过、对抗、隐藏或探测任何**反外挂**机制；
* **没有**注入、挂钩、线程操作或**写内存**；
* **没有**把任何取证产物**提交入库**。

### 8.3 产物在哪里（可复核）

全部取证产物落在一个**仓库之外**的工作目录。它**不在**本仓库内，也**不在**本仓库的任何提交里。

> **这一节不写出该目录的名字**：它属于本仓库之外的记录范围；而且**把名字写进本文档，`git grep` 就会命中它自己**（自指）。
> 要核的是「**它在不在仓库里**」，下面第一条判据就够了，**不需要名字**。

```powershell
# 取证工作目录是本仓库的兄弟目录，不在本仓库内（占位符请自行替换）
Test-Path <本仓库>/<取证工作目录>          # 应为 False

# 本仓库跟踪文件总数（写作时为 306，随仓库增长而变，不是判据）
(git -C <本仓库> ls-files | Measure-Object -Line).Lines
```

### 8.4 这一节的边界

* 这些产物**不进入本仓库、不随本仓库分发** ⇒ **不构成本项目对游戏数据的再分发**；
* 取证的目的是**验证本项目自己的模拟模型是否与真实行为一致**，**不是**复制或替代游戏的任何资源；
* **不保证**这类做法在任何司法辖区都毫无争议；**使用者应自行确认所在地法律与自己账号的许可协议**；
* 本项目与鹰角网络的隶属与授权关系见**文首说明**：**无隶属、未获授权、不提供任何游戏资源下载**。
