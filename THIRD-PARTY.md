...
| --- | --- | --- |
| 读取运行中客户端的进程内存（唯一取数手段） | 操作系统公开的**只读**进程内存接口；**只申请读权限与查询信息权限** —— **无写、无注入、无挂钩、无线程操作** | **否** |
...
```powershell
# 本仓库的跟踪文件里不应出现取证工作目录的名字（本行应无输出）
git -C <本仓库> ls-files | Select-String -Pattern "realtime-probe"

# 该工作目录不在本仓库里（应为 False）
Test-Path <本仓库>/realtime-probe
