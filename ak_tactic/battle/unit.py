...
class OperatorUnit(Combatant):
    ...
    #: 这位干员的**天赋战术点/召唤物** char_id（从 `Talent.token_key` 抄来）。
    #:
    #: 全库 37 位有此字段（凯尔希→Mon3tr、可露希尔→指挥中心、深海色→触手…）。
    #: 战斗层要靠它判「战场上那个单位是不是我的战术点」——**按 char_id 判**，
    #: 不能按名字或描述关键词猜（`Talent` 上原本没带这个列，2026-09-19 补）。
    token_key: str = ""
    #: 这次部署吃到过「返还部署费用」没有（可露希尔技2 的【返费】）。
    #: 单独一个字段是为了让守卫能**直接看这个可观测量**，而不是去比费用池——
    #: 费用池会被别的回费机制同时改，比不上。
    cost_refunded: bool = False
    #: 这次带的**技能槽号**（1/2/3；0 = 没带技能或认不出来）。
    #:
    #: `op.skill` 是个 `SkillLevel`，**它自己不带槽号**，而有些机制要按槽查——
    #: 例：可露希尔的战术点「指挥中心」三个技能各给一个效果范围（技1→`x-5`、
