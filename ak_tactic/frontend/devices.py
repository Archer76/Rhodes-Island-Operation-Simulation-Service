...
#: ⚠ **本模块整体从 `ak_tactic/battle/devices.py` 搬来**（原样搬，不重写）。
#:
#: 搬的理由是依赖方向：`environment.py` 依赖它，而 `environment.py` 又要给
#: `spec.py`/`mech.py` 用。若让新家反过来 import `battle/`，就等于
#: "新家依赖要删掉的 `battle/`"，白搬。
#:
#: `battle/devices.py` 现在是一层**转出壳**，所以
#: `from .devices import DeviceUnit` 与 `ak_tactic.battle.devices:BLOCKER_KEY`
#: 这类既有写法**一个字都不用改**。
from .mech_consts import (                                  # noqa: E402
