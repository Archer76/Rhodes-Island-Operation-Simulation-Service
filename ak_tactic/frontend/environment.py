...
#: ⚠ **本模块整体从 `ak_tactic/battle/environment.py` 搬来**（原样搬，不重写）。
#:
#: 搬的理由是依赖方向：`spec.py`/`mech.py` 送规格时要用 `FarmlandSystem`/`Field`/
#: `PolluteParams` 与十条常数量，若让新家反过来 import `battle/`，就等于
#: "新家依赖要删掉的 `battle/`"，白搬。
#:
#: `battle/environment.py` 现在是一层**转出壳**，所以
#: `from .environment import FarmlandSystem` 与
#: `ak_tactic.battle.environment:RUNES_KEY` 这类既有写法**一个字都不用改**。
