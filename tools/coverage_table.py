...
ROWS: list[tuple[str, str, str, str]] = [
    ...
    # ⚠ 2026-09-20 PM 裁定：原「闪避／伤害抵挡」**拆两行**（族名里两半方向不同：
    #   闪避＝受击方针对某伤害类型的比例；伤害抵挡＝结算函数的 `dodge` 参数）
    ("敌人", "闪避", "rios-sim/skill.go::dodgeVs",
     "闪避不生效 ⇒ 受击方被静默算成**必中**（伤害期望值通道失效。本行**已接线**；⚠ 敌人侧 `sim.go:239` 恒 0 是"
     "「与原版同值的零」，不是「零调用点」）"),
    ("敌人", "伤害抵挡", "rios-sim/sim.go::resolveDamage",
     "抵挡不生效 ⇒ 整笔伤害落到血上（本行**已接线**：期望值法、真伤不吃、先保底后闪避）"),
    # ── 干员平面
...
KINDS: dict[str, str] = {
    ...
    "元素损伤（族）": "调用点缺失",
    # ⚠ 2026-09-20 PM 裁定：原「闪避／伤害抵挡」的「概念未建模」**已删除**——它与"三支都有定义位/消费点/守卫"
    #   直接矛盾；且该族已拆成「闪避」「伤害抵挡」两行、两行都判「已接线」⇒ 无病型可写。
    #   **不许**保留原值＋注冲突（理由同「晕眩」：栏位是给下一个人看的，依据栏不是每个人都读）。
...
SEVERITY: dict[str, str] = {
    # ⚠ 2026-09-20：随行拆分而拆（严重度沿用原行「必败级」，我**未**新判严重度）
    "闪避": "必败级", "伤害抵挡": "必败级", "屏障／层数护盾": "必败级", "晕眩": "必败级",
...
#: ⚠ **表名说的是"我们没有"，但表里已经有几行经复核发现"其实有"**
#:   （`闪避`／`回复`／`治疗`／`冷却·元素损伤爆发`）。
#:   **这些行的「说明」栏自己写着「原句已作废」与真实落点——别按表名读它们。**
#:   留着不移出是 PM 2026-09-20 的裁定：**保留那个普查测量、改掉那句解释**。
UNMODELED_BY_SEVERITY: list[tuple[str, str, str, str]] = [
    ...
    ("数值偏移", "回复", "280 条",
     "★ **本行原句「概念未建模：治疗量无统一字段族」的因果已作废**（PM 2026-09-20 路由本单核正；"
     "**保留「280 条」这个普查测量、不移出**——同「闪避」那一裁的口径）。"
     "「回复」这个词在语料里**混着两件事**，压成一句结论就是错的，必须分开写："
     "① **回血**——落点与接线状态见下一行「治疗」，**不重复判**；"
     "② ★ **回技力**（语料原话如「额外回复 3 点技力」，标本 `char_002_amiya`）——"
     "⚠ **这一支在 Python 侧是已建模的**：定义位 `ak_tactic/battle/talents.py:398`（`def find_sp_on_action`）、"
     "消费点 `ak_tactic/battle/sim.py:3401`（`spa = find_sp_on_action(op.talents)`）、"
     "守卫 `tools/check_battle.py:859`。"
     "**它缺的不是模型，是「没送进规格」**：`ak_tactic/simgo/spec.py:258-259` 按天赋 `find_sp_on_action` "
     "**主动挡下**（`bad.append(天赋回技力…)`）⇒ 规格侧根本不送；`rios-sim/` 下该符号**零命中**。"
     "⚠ 属「**已登记的缺口**」（`tools/gate_inventory.py:79` 登记，`docs/gate-inventory.md:81` 一行写的就是「**未送**」），"
     "**病型＝数据到不了**，**不是「模型里没有这个概念」**——两者修法不同，别读成一个。"),
    ("数值偏移", "治疗", "209 条",
     "★ **本行原句「概念未建模：与回复同族」已作废**（PM 2026-09-20 路由本单核正）。"
     "**治疗量有字段族；真正的缺口是其中一支没进规格。**三句话："
     "① **医疗干员的平A治疗＝已接线**，而且**这句话是仓库自己写下的**——"
     "`ak_tactic/simgo/spec.py:247-248`：医疗的平A（`heals`）**已经接线**"
     "（Go 的 `operatorsAttack`：判这一击有没有被技能改成伤害、按 (血量比例, 血量) 选最低的治疗、回复量夹生命上限）。"
     "坐标：定义位 `rios-sim/sim.go:432`（`func (o *operator) heal`）；"
     "消费点 `rios-sim/sim.go:1201`（技能治疗）／`:1945`（平A 给友方，目标选择 `:1888 pickHeals`、定义 `:2176`）／"
     "`:2410`（自然回复）；门槛 `:1884`（`deals := !op.spec.Heals || …`）；规格字段 `rios-sim/wire.go:210 Heals bool`。"
     "② ★ **真缺口＝技能自己写的 `heal_scale` 没进规格**：Go 只做「平A 倍率 1.0」这一种；"
     "Python 侧照样解析得到（`ak_tactic/operator/skill.py:1293`、`ak_tactic/formula.py:532`），"
     "但规格侧**主动挡下**（`ak_tactic/simgo/spec.py:253-255`）⇒ 病型属「**数据到不了**」，**不是「概念未建模」**。"
     "已在 `tools/gate_inventory.py:65` 登记（`技能治疗倍率 heal_scale`）。"
     "③ 「受到的治疗量+20%」是**另一族**：`ak_tactic/formula.py:810-811 heal_received_up` ——"
     "⚠ 全仓**只此一处命中**（`ak_tactic/` 与 `rios-sim/` 均无第二处）⇒ **解析得到、零消费**。"),
    # ⚠ 2026-09-20 PM 裁定：原「冷却」一行**拆两行**（口径同「闪避／伤害抵挡」那一裁）。
    #   拆的理由：① 它把「已建模的一族」与「没建模的一族」**压成了一行**；
    #   ② ★ 而它用来给这一行命名的那个词（「技能冷却」）**在语料里 0 条命中** ——
    #   **它连「自己到底在数什么」都没说清**。
    #   ⇒ 「技能冷却」**不单独立行**（它不是语料类别，是原行的错误命名）；
    #   **原行名与拆行动作在两行说明栏里都留痕**（同 `c495206` 给回退路径留痕是同一个动作）。
    #   ★ 条数自洽锚：**4 + 3 = 7 = 原行条数**。
    ("数值偏移", "冷却·元素损伤爆发", "4 条",
     "★ **本行与下一行是原「冷却」行按 PM 2026-09-20 裁定拆出来的两半**（口径同「闪避／伤害抵挡」那一裁）。"
     "⚠ **原行名「技能冷却」在语料里 0 条命中**——它不是语料类别，是原行的**错误命名**，"
     "**故不单独立行**；原句「概念未建模：技能冷却口径未建 ⇒ 技能可用时刻偏移」的**因果已作废**。"
     "★ 条数自洽锚：**4 + 3 = 7 = 原行条数**。"
     "**这一半是已建模的**：`rios-sim/element.go:266 cooling`／`:270 anyCooling`；"
     "★ **接线状态以具名行「元素损伤（族）」为准**（`KINDS` 记「调用点缺失」），**本行只报普查条数**。"
     "语料那 4 条是：「侵蚀损伤冷却恢复速度+25%」（`char_4223_botany`）／"
     "「神经损伤冷却恢复速度+50%」（`char_1042_phatm2`）／「侵蚀损伤冷却减少1秒」（`char_4235_thumpy`，2 条）。"),
    ("数值偏移", "冷却·天赋内部 ICD", "3 条",
     "★ **本行与上一行是原「冷却」行拆出来的两半**（拆行理由、原行名 0 条命中、以及「7 = 4 + 3」见上一行，不重复）。"
     "**这一半是真缺口**：**Go 侧没有「天赋内部冷却（ICD）」这个概念。**"
     "★ **这句话的取证范围（按本项目「写零命中前先报范围」的规矩）**："
     "全 `rios-sim/**/*.go`（含测试）对 `(?i)icd|internal_cooldown|cooldown` 只有 **2 处真实命中**——"
     "`element.go:266 cooling` 与 `element_test.go:64 TestElementCooldownBlocksEveryElement`，"
     "**两处都属上一行那一族**；**没有任何「天赋内部冷却」的字段、判据或计时器**。"
     "语料那 3 条全是 `char_4042_lumen` 的同一条天赋、按精 0／1／2 各一条"
     "（「攻击范围内的友方受到异常状态时，立刻治疗…该效果有 12 秒／11 秒／10 秒冷却时间」）。"
     "**病型＝概念未建模**（**要先立字段，不是补一行调用**）——与上一行的「元素损伤冷却」**不是同一族、修法也不同**。"
     "⚠ **本行未核**：Python 侧是否已有天赋 ICD 的对应实现（本单只判 Go 侧无此概念）；"
     "也**未核**「干员的技能可用时刻」在模型里由哪条链管"
     "（⚠ 若将来有人想为它立行，**先核**——别照「技能冷却」这个名字再立一行）。"),
]
...
#: ★★ **总则（PM 2026-09-20 三行裁定时立，为的是同类冲突不再来第五次）**：
#: **本表 `WIRED` 一律指「引擎／规格**消费**这个量」，不指「解析器／工具能跑」「有守卫」「有定义位」。**
#: 理由：这张表已被 `晕眩`、`闪避／伤害抵挡`、`关卡机制解析` **三处**证明会漂，而且漂的方向都一样——
#: **把「有定义／有守卫／命令行能用」读成了「引擎接线」**。
#: ⚠ 配套口径：**「有多条消费链」与「有专门的守卫用例」是两件事**；后者属**登记缺口**，**不进 `WIRED` 取值**。
WIRED: dict[str, str] = {
    ...
    # ⚠ 2026-09-20 **三行裁定**（PM 在证据上裁；连同上面的总则一起读）：
    #   ① 原「闪避／伤害抵挡」拆两行，**两行都是「已接线」**（我方闪避 `skill.go:327` → 消费点 `sim.go:1319`；
    #      伤害抵挡＝`sim.go:3133-3134 resolveDamage(…, dodge)` 的期望值法）；KINDS 的「概念未建模」按裁定**删除**。
    #   ② 「天赋闪避／伤害抵挡」→ **已接线**（消费链 `spec.py:523` → `:792` → `wire.go:176-177` → `skill.go:327-338`
    #      → `sim.go:3133-3134` 是通的）；两条缺口（这一路**无专门用例**／规格侧是否发非 0 值**未核**）
    #      **登记在依据里，不进本栏**——把守卫缺口塞进接线栏正是总则要防的。
    #   ③ 「关卡机制解析」→ **未接线（零调用点）**（引擎侧零消费；只有 `cli.py:1686`／`:1691` 命令行通路），
    #      与「装置层（devices 门）」同型；「解析器可跑且有 7 个 check_ 守卫」作为**独立事实**留在依据里。
    "闪避": "已接线", "伤害抵挡": "已接线",
    "天赋闪避／伤害抵挡": "已接线", "关卡机制解析": "未接线（零调用点）",
...
WIRED_PENDING_RULING: dict[str, str] = {
    # ★ 2026-09-20：原先登记的三条（闪避／伤害抵挡、天赋闪避／伤害抵挡、关卡机制解析）**已全部裁定并改值**
    #   （PM 三行裁定 + 总则），故本登记位**清空**。保留这个字典，是为了下一次出现背离时还有地方放
    #   「已上报、等裁定」——登记的每一条都必须是已上报的，裁定回来后改值并从这里删掉。
}
...
WIRED_WHY: dict[str, str] = {
    ...
    "晕眩":
        ...
        "Go 侧还没有 down / stun / idle / disarm 这四种」。⚠ **这条曾与本表 `WIRED` 的那一栏冲突（原值「已接线」）**——"
        "**已按 PM 2026-09-20 裁定改值**：本行现在写「未接线」，与上面两条自述、以及整族穷举同向（PM：假栏位比没栏位更糟）。"
    ...
    "元素损伤（族）":
        ...
        "★★ **两态量测（PM 2026-09-20 第四件要求：潜在风险 vs 实害，两边都要报）**——"
        "**① 潜在风险＝有**：走数据侧 `character_table.json`（天赋黑板）＋`skill_table.json`（技能黑板，经 `skills[].skillId` 映射回干员），"
        "**携带元素／损伤相关黑板键的干员共 32 个，其中 15 个携带 Go 匹配不到的键**；按「损伤量」与「抗性/其他」再分："
        "损伤量一族＝`attack@extra_ep_damage_scale`（char_4146_nymph、char_499_kaitou）、`attack@ep_damage_ratio_talent`（char_487_bobb）、"
        "`ep_damage_ratio_token`（char_1042_phatm2、token_10054_phatm2_encdool）、`ep_damage_ratio[damage]`／`[trigger]`（char_4235_thumpy）、"
        "`ep_damage_ratio_m`（char_4098_vvana）、`attack@ep_damage_ratio_normal`／`_win`（trap_186_cnvfwks）；"
        "**非损伤族（不与 `EpCandidates` 同量，Go 不匹配它们是对的）**＝`ep_damage_resistance`（是**抗性**，另有 `[other]`／`[water]` 限定）、"
        "`element_atk_scale`／`element_damage_scale`／`element_multiplier`／`element_type`。"
        "**② 实害＝0（按「谁会真的出场」量过）**：⚠ **第一次量法我搞错了关键字空间**——我按 `char_…` 文本去搜 `fixtures/`，而放行计划的部署项写的是"
        "**中文干员名**（`plan-*.json` 的 `{\"operator\": \"能天使\", \"position\": …, \"skill\": …}`）⇒ 那次「交集为空」**不作证据**。正确量法：把部署名经 `character_table.json` 的 `name` 字段解析回 charId"
        "（**未解析的名字 0 个**），再与「携带未匹配键的 15 名」求交：**17 份 `plan-*.json` 只涉及 7 个 charId、名册 `roster_max_modelled.json` 20 项**，两者交集**均为空**；"
        "**逐份计划统计「这份计划会带进来的未匹配键」，合计 0 处** ⇒ 已放行关卡上 `EpCandidates` 不会遇到这些键。"
        "（附带事实：放行计划的部署项是**名字**、名册是 charId——两套标识，混用会得出假结论。）"
        "⚠ **这条量测的边界（别读成「永远安全」）**：① 它只覆盖 `fixtures/` 当前的 21 个夹具与本次走的两张表（**没走** `battle_equip_table.json` 的模组、"
        "也没走敌人侧），换名册／加关卡／加模组都可能把某个携带者带进来；② 该族在引擎里零消费点，所以即便带进来，**今天也没有可观测量**。"
        "⇒ 结论口径：**差集是「潜在」风险、不是「实害」**；触发条件已写明（携带者进入夹具），下次量测照这两态跑。",
    # ---- 批次四（收尾）：2026-09-20 PM 三行裁定 + 总则；本族按裁定**拆两行** ----
    "闪避":
        "锚点 `rios-sim/skill.go:327`（`func (o *operator) dodgeVs(damageType string) float64`；PM 2026-09-20 裁定指定的锚点，"
        "原锚点 `sim.go:239` 只是族里的一半）；**消费点** `sim.go:1319`（机制直伤那一路把 `op.dodgeVs(damageType)` 传进结算）"
        "⇒ 引擎**真的消费**它，本行判「**已接线**」。"
        "★ **裁定要求照写的那句（敌人侧那一半）**：`sim.go:239 func (e *enemy) dodgeVs(string) float64 { return 0 }` ——"
        "**恒 0 且与原版同值**：原版 `battle/sim.py:3921` 读 `target.dodge_phys + target.talent_dodge_phys`，"
        "而 `EnemyUnit.dodge_phys` 是个**没有任何地方写过**的字段、敌人侧也没有天赋抵挡那一对字段 ⇒ "
        "**这是「两边一致的零」，与「零调用点」必须分开写**（零是「双方同值」，不是「没人调用」）。"
        "敌人侧那三个消费点仍在（`sim.go:1238` 敌人 take、`:1483`、`:1969`），恒 0 的理由写在 `sim.go:233-238`。"
        "⚠ 原 `KINDS` 值「概念未建模」与实测矛盾，已按裁定**删除**（改值，不保留＋注冲突）。"
        "锚点指向的是哪个量：`dodgeVs`（operator 那个）是「**我方干员**针对某伤害类型的闪避比例」——本行只判这一半。",
    "伤害抵挡":
        "锚点 `rios-sim/sim.go:3133-3134`（`func resolveDamage(atk float64, damageType string, scale, defense, res, dodge float64) float64`；"
        "PM 2026-09-20 裁定指定的锚点）；语义 `sim.go:3126`：`dodge` 是受击方针对该伤害类型的闪避比例，"
        "**原版把它当期望值**（期望值法，不是掷骰）⇒ 引擎**真的消费**它，本行判「**已接线**」。"
        "**守卫**：`damage_test.go:13 TestResolveDamageDodge` 四支俱全——`:16` 物理折扣、`:20` 法术各取各的、"
        "`:25` **保底与闪避的次序**（先保底后闪避）、`:30` **真伤不吃闪避**（对应原版 `damage.py:144`）。"
        "锚点指向的是哪个量：`resolveDamage` 的 `dodge` **参数**——族名里「伤害抵挡」的那一半"
        "（「闪避」那一半见上一行，锚点在 `skill.go:327`）。",
    ...
    "天赋闪避／伤害抵挡":
        ...
        "★ **2026-09-20 PM 裁定**：这条消费链是通的、且结算那一步有 `TestResolveDamageDodge` 四支 ⇒ **本行判「已接线」**。"
        "⚠ **两条缺口照写，但都不进 `WIRED` 取值**（总则：把守卫缺口塞进接线栏，正是要防的那件事）——"
        "**(a)** 「天赋闪避这一路接上线」**没有专门用例**（Go 侧 `Dodge` 一词只命中 `damage_test.go` 这一个用例）；"
        "**(b)** 规格侧**会不会发出非 0 值未核**（要看名册里哪些干员带这对天赋）⇒ 登记为**缺口／未核**，不据此改栏位。",
    "关卡机制解析":
        ...
        "**消费点：引擎侧（`battle/`、`simgo/`）零调用点**——全 `ak_tactic/**/*.py` 里除 `mechanics.py` 自身外，"
        ...
        "行名「关卡机制解析」指的就是它。"
        "⚠ 原 `WIRED` 写「已接线」，与「引擎侧零消费」并置时含义不清（是「解析器能跑且有守卫」还是「引擎消费它」？两种情况结论相反）。"
        "★ **2026-09-20 PM 裁定**：按总则（`WIRED` 只指引擎／规格**消费**）改判「**未接线（零调用点）**」，与「装置层（devices 门）」同型。"
        "★ **独立事实（它真实的价值，但不是接线）**：解析器**可跑**，并且有 7 个 check_ 的守卫"
        "（`tools/check_mechanics.py` 418 行；`:233`／`:238`／`:248` 直接调被测函数）——这条留在依据里，**不进 `WIRED`**。",
}
...
def red_names(assessed: list[dict]) -> list[tuple[str, str, str]]:
    ...
    ⚠ **引号字形经统一（原文为 ASCII 引号），词句一字未改**（PM 2026-09-20 裁定：判据管的是**字形**，
    读者需要的是**词句**，两者分开声明就不必任何一方牺牲）。字形统一的批量改动见 `868594c`。
