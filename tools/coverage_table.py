...
WIRED_WHY: dict[str, str] = {
    ...
    "元素损伤（族）":
        ...
        "**② 实害＝0（在本次量测范围内）**：拿这 15 个干员去比对 `fixtures/`（共 **21 个 JSON**，本项目唯一判据集）——"
        "**一个都没出现**（`char_4114_harold`／`char_4225_tanya`／`char_487_bobb`／`char_4146_nymph`／`char_1040_blaze2`／`char_1042_phatm2`／"
        "`char_1016_agoat2`／`char_4235_thumpy`／`char_4098_vvana`／`char_499_kaitou`／`char_473_mberry`／`char_4204_mantra`／`trap_186_cnvfwks`／"
        "`token_10054_phatm2_encdool` 等，与 21 个夹具的文本交集为空）⇒ **在已放行关卡的夹具上，`EpCandidates` 不会遇到这些键**。"
