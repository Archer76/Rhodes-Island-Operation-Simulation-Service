...
#: ⚠ **表名说的是"我们没有"，但表里已经有几行经复核发现"其实有"**（`闪避`／`回复`／`治疗`）。
...
UNMODELED_BY_SEVERITY: list[tuple[str, str, str, str]] = [
    ...
    ("数值偏移", "冷却", "7 条",
     "★ **归因已核正**（PM 2026-09-20 要我对本行作判断）。原句「概念未建模：技能冷却口径未建」"
     "**指错了东西**——逐条核过 `data/prts-notes.sqlite` 的 `fact` 表，那 7 条里"
     "**没有一条讲技能冷却**："
     "**4 条是元素损伤爆发的冷却**（「侵蚀损伤冷却恢复速度+25%」「神经损伤冷却恢复速度+50%」"
     "「侵蚀损伤冷却减少1秒」；`char_4223_botany`／`char_1042_phatm2`／`char_4235_thumpy`）——"
     "**这一半是已建模的**（`rios-sim/element.go:266 cooling`／`:270 anyCooling`，见「元素损伤（族）」行）；"
     "**3 条是天赋自己的内部冷却 ICD**（`char_4042_lumen`「该效果有12秒／11秒／10秒冷却时间」）——"
     "**这一半才是真缺口**（Go 侧无 ICD 概念）。"
     "⇒ ★ **本行把「已建模的一族」与「没建模的一族」压成了一行，而它写的那一族在语料里 0 条命中。**"
     "**是否拆两行请项目经理裁定**（拆行口径参照 2026-09-20「闪避／伤害抵挡」那一裁）；"
     "**本次未拆**——拆行是裁定权限，本单只把归因核正。"),
]
