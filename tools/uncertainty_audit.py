...
_Q: list[tuple[str, str, str, str]] = [
    ...
    ("`ep_damage_scale` 一名多义：同一个键名至少三种语义",
     "三种语义各有实例：①元素**伤害**＝攻击力×N（`skchr_warmy_2`，0.5）；"
     "②**损伤倍率**（`skchr_phatm2_1`，1.5 倍）；③**伤害基数**比例（装置 `trap_221_ftshad`「空壳」，0.4）。"
     "**实测落点（2026-09-20，直接跑 `parse()` 取证，不靠规则名推断）**："
     "「爆发期间则对其额外造成相当于攻击力{attack@extra_ep_damage_scale:0%}的元素伤害」"
     "→ `kind=ep_burst source=ep_burst_extra2 expr=ATK × 40%`；"
     "「造成相当于攻击力{attack@ep_damage_scale:0%}的元素伤害」→ `kind=damage source=damage_atk`；"
     "「…相当于攻击力{attack@ep_damage_scale:0%}的伤害」→ `kind=damage source=damage_bare`。"
     "⇒ **同一个键名在不同句式下落到不同的 kind/source**，而句式本身**不区分**这三种语义。"
     "⚠ 本节的前提曾被写成「一条规则都没有 ⇒ 既没编译出公式项、也不进未识别清单」——"
     "**实测否证**：三句都编译出了公式项。真正的问题是**落点**，不是漏编译。",
     "`ak_tactic/formula.py` 里**没有以 `ep_damage_scale` 命名的规则**；该键只作为黑板值被上述句式消费。"
     "所以现值不是「没编译」，而是「**编译成了语义更弱或不同的项**」——"
     "`damage_bare` / `damage_atk` 都不带元素损伤语义，`ep_burst_extra2` 只覆盖爆发期那一支。"
     "对拍影响：执行 B 组「黑板原样映射」时**不许按键名猜语义**，必须先看正文；"
     "否则同一个键名会在不同单位身上被算成不同的量。",
     "**待裁——本项不预设处理方式**。可裁的选项："
     "① 维持现状（按键名透传，由消费侧按正文上下文决定）；"
     "② 为三种语式各加一条**显式**规则并配守卫（让「按键名猜语义」的写法变红）；"
     "③ 先逐条取证「空壳」「warmy」「phatm2」三处的实际结算再定。"),
