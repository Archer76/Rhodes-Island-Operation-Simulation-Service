...
SUITE = [
    ...
    #: ★ 2026-09-24 加（第二十六套）：**命令面**。
    #: 它不是 Go vs Python 的对拍，判的是「Python CLI 在**缺值输入**下不许崩」——
    #: 起因是 `stage main_02-07` 崩在 `None.__format__('g')`（敌人「御4」的重量等级
    #: 在游戏本体里没覆写 ⇒ 读出 `None`），而那个形状**长得像「数据缺失」**。
    #: 取证面与两处对照都**现算**（缺值敌人集合 → 引用它的关卡当正例、
...
def cached_content_count(levels: list[str]) -> int:
    ...
    ★ 现算（2026-09-24）的细分，读的人要知道它比「`#f#` 对」更宽：
    320 个键 → 159 份 `data_path`；**144 组**共用同一个 `data_path`，
    多出 **161** 个键。这 144 组里**有 15 组不是 `#f#`**，而是
    `easy_XX-YY` 与 `main_XX-YY` 这对**别名**（同一份内容、两个关卡键）——
    只按「`#f#` 对」去数会漏掉它们。所以口径取「按 `data_path` 去重」，
    与「剥掉标签字段后按内容去重」两条路实测同值（159／159）。
