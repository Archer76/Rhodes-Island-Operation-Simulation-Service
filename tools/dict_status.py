...
NOT_A_FLAG: dict[str, str] = {
    ...
    "折射": "术语 `ba.refraction`（敌人侧独有），**权威页 `异常效果` 全文零命中** ⇒ 不是异常效果。"
            "⚠ **判读已更正（2026-09-20，取自 `data/enemydb.sqlite` 的游戏自身数据）**："
            "词典写「生效时法术抗性+70」，那只是**深池暗影术师/队长**那一个载体的效果"
            "（`enemy_level.talent`：「折射：法术抗性增加70（**可被沉默**）」）；"
            "**假想敌：镜膜**身上折射给的是「最大生命值+100%（受沉默影响后失效，**无法恢复**）」。"
            "⇒ 折射是个**状态/术语**，**生效时做什么由载体天赋决定**，「+70」是实例不是定义",
...
ANCHORS: dict[str, tuple[str, str]] = {
    ...
    "折射": ("rios-sim/refraction.go::refractionState", "状态层：Active/Silenced/Latched 三件分开存；"
             "效果量由调用方给（不许把 +70 写成常量）；镜膜式「沉默后无法恢复」用 latched 兑现"),
}
