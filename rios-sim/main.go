...
func handle(req *request, started string) response {
	...
	case "costof":
		// 丙阶段四·第二十一批：部署费用（见 `deploycost.go`）。
		if len(req.Spec) == 0 {
			return response{ID: req.ID, OK: false,
				Error: "costof 少了 spec（一批 OperatorCalcConfig）"}
		}
		var cfgs []OperatorCalcConfig
		if err := json.Unmarshal(req.Spec, &cfgs); err != nil {
			return response{ID: req.ID, OK: false,
				Error: fmt.Sprintf("spec 不是参数数组：%v", err)}
		}
		vals := make([]int, 0, len(cfgs))
		for _, c := range cfgs {
			n, err := CostOf(c)
			if err != nil {
				return response{ID: req.ID, OK: false,
					Error: fmt.Sprintf("%s：%v", c.CharID, err)}
			}
			vals = append(vals, n)
		}
		raw, err := json.Marshal(vals)
	...
	case "arrivals":
		// 到达表（`eta.enemy_arrivals` ＋ `ArrivalIndex`，见 `arrivals.go`）。
		// 关卡走 req.Level 或 req.Path（合成关卡），与 `routeplans`／`spawns` 同口径。
		var aq ArrivalsQuery
		if len(req.Spec) > 0 {
			if err := json.Unmarshal(req.Spec, &aq); err != nil {
				return response{ID: req.ID, OK: false,
					Error: fmt.Sprintf("arrivals 的 spec 解不开：%v", err)}
			}
		}
		aout, err := ArrivalsOf(req.Level, req.Path, aq.Difficulty, aq.SpeedScale,
			aq.Cells)
		if err != nil {
			return response{ID: req.ID, OK: false, Error: err.Error()}
		}
		raw, err := json.Marshal(aout)
		if err != nil {
			return response{ID: req.ID, OK: false,
				Error: fmt.Sprintf("序列化失败：%v", err)}
		}
