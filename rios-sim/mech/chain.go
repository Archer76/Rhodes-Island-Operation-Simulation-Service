...
// 第 3 跳是等比（0.75² = 0.5625）、等差（1 − 0.25×2 = 0.5），还是**每跳倍率（0.75 本身）**
// ——**三个候选**，而语料只给了 `0.75` 一个数、n=2 **三读法完全同值** ⇒ **未定**。
// 判错的代价是把某一读法**焊进实现**（不可逆，同族 `b97c4132`），所以本文件
// **不实现第 3 跳**：算到那一跳就返回 `ErrJumpUndetermined`。
// **未定是一个带出口的状态**，出口写在 `docs/chain-skeleton-plan.md` §七（7.2 是前置未定项）。
...
//
// ★★ **候选是三个，不是两个**（PM 裁定 A 时点名要求保留醒目，见 `docs/chain-skeleton-plan.md` §七 7.2）：
//
//	① 等比衰减（0.75 是衰减系数）⇒ 第 3 跳 0.5625 × 首跳
//	② 等差衰减（每跳减 25% 首跳）⇒ 第 3 跳 0.5 × 首跳
//	③ **每跳倍率**（0.75 就是倍率本身）⇒ 第 3 跳 0.75 × 首跳
//
// 三者在 n=2 **完全同值**（都是 0.75 × 首跳）⇒ 本文件只断言到第 2 跳**不是因为懒，
// 而是因为第 2 跳对「区分读法」零信息量**；分岔点只有 n=3 一处。
var ErrJumpUndetermined = errors.New(
	"chain: 第 3 跳及以后的读法未定（三个候选在 n=3 分岔：等比 0.75²=0.5625 / " +
		"等差 1-0.25×2=0.5 / 每跳倍率 0.75，而语料只给了 0.75 一个数）；" +
		"按纪律不猜，出口见 docs/chain-skeleton-plan.md §七")
...
//
// ★ 关于 n = 2 为什么可以断言：**三个**读法（等比衰减／等差衰减／每跳倍率）在 n=2
// 恰好重合（都是 0.75 × 首跳），**分岔从 n=3 才开始**（0.5625 ／ 0.5 ／ 0.75）。
// 这也是判据只判到第 2 跳的全部理由——**第 2 跳对「区分读法」零信息量**，
// 而这句话有两面：它既让第 2 跳成为唯一敢断言的一点，也说明它证不出任何读法。
