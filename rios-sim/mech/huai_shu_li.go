...
// 都真的用得上——不是"代码写得出来"就算数。原先这里列了两处**没接线**，现在只剩
// 装置层那一条（`simgo/mech.py` 的 `port_reasons` 自 2026-09-18 起**恒为空**，
// 那一条改由 `spec.py` 的**关卡装置**闸门覆盖）：
//
//   - 敌人侧的污染来源（被击倒的 `passive_pollut` 等，原版 `sim.py:2221` 的
//     帧位置）——**已接线**：`PostAttacker` 实现 `huai_shu_li.go:1559`、接线
//     `sim.go:965`（原注释写"钩子还没加"，与实现相反，2026-09-20 更正）；
//   - 装置的运行期行为（田鼷拆阀 → 地形还原、天桩链）。
//
// 这些事都只在"点名了本机制、又用到了它们"时才危险，所以闸门按**这一关实际
// 用到的东西**逐条判，而不是一刀切。
