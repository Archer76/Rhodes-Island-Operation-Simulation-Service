...
### 阶段 7 · 练度前十干员的战斗建模 🔄 进行中

搜索器只对「已经建模的机制」负责，所以先把博士练度最高的十名干员逐个核过一遍。
**「已建模」的判据是 `ak_tactic/battle/` 里有对应结算**——只在 `formula.py` 里
编出公式项、战斗层没人消费，不算数（闪避就是这么漏掉的，见下）。

名单（按 精英↓ 等级↓ 潜能↓ 名册序；阿米娅两形态合占一位）：
电弧 `char_4195_radian`、机械师 `char_4230_mcnist`、望 `char_2027_wang`、
赤刃明霄陈 `char_1050_chen3`、圣聆初雪 `char_1046_sbell2`、令 `char_2023_ling`、
阿米娅 `char_002_amiya` + `char_1001_amiya2`、提丰 `char_2012_typhon`、
逻各斯 `char_4133_logos`、予愿安洁莉娜 `char_1015_aglna2`。

**第一批（不需新系统的机制）**——八项逐条核验结果：

| 机制 | 状态 | 落点 / 说明 |
|---|---|---|
| 全场属性光环 | ✅ | `battle/talents.py`，随帧刷新（光环主人开技能时加倍） |
| 伤害类型改写 | ✅ | `skill._wants_true_damage` 判据锚在「伤害类型变为真实」 |
| 无视法抗 | ✅ | `damage.resolve_damage(ignore_res=)` |
...
1. `formula.py` 的三条规则写的是 `获得{N}{PCT}**的**物理闪避`，
   而正文里带「的」与不带「的」**各占一半**（赤刃技2 原文就没有「的」）
   → 编译层一个项都吐不出来；
2. `SkillEffects` **没有** dodge 字段，`skill.py` 里一处闪避处理都没有；
3. 战斗层无结算。

已补齐的三段：`formula.py` 三规则改 `的?`（同文件的 `pen_res` 早有先例）；
`skill.py` 新增描述驱动判据 `_wants_dodge` 与 `SkillEffects.dodge_phys/arts`；
`battle/damage.py` 加 `dodge_phys/dodge_arts` 参数、`unit.OperatorUnit` 存字段、
`sim.py` 开/关技能时置位与清零、敌方出手时传入。

**闪避走期望值法**（博士 2026-09-17 裁定④）：把最终伤害乘 `(1 − 闪避率)`，
**不掷骰**。掷骰会让同一份作业每次跑出不同结果，搜索与回归都不可复现。

三条**故意不认**的写法（宁可少认，不要认错），已记进 `docs/uncertainties.md`：
`获得30%的**近战**物理闪避`（火神，限定只挡近战）、
`有15%的概率闪避敌人的近战物理攻击`（因陀罗，结构不同且成功后还加成）、
`治疗友方单位后为其**提供**…物理闪避`（斑点，作用对象是被治疗者）。

⚠️ `unit.OperatorUnit.dodge_*` 的判据必须走**描述**：赤刃技2 的值在黑板键
`chen3_s2[respawn_buff].prob` 上，而**同一个 `prob` 名字**在提丰技2 里是
`attack@prob` = 40% 概率晕眩。**键名同名反义**，按键名认必错。

⚠️ 闪避已能取到 60%，但赤刃技2 的**整套斩击+位移**仍未建模（属第二批的「位移」），
所以目前没有任何内置作业会真正触发它。同理 `variants["respawn_buff"]`（技2 的
+300% 攻击力）**无消费方**——战斗层只读 `variants["kill"]`。

**第二批（大件，未开工）**：召唤物、屏障/护盾、状态机（寒冷/冻结/战栗/晕眩/
沉默/束缚/法术脆弱/虚弱）、弹药、位移、随机性。
**元素损伤**中数值不确定的暂不做。收尾时汇总一份待博士寻找数据源校正的清单。

**回归纪律**：每一步都要跑全套自检，并保持三条基线**逐字不变**——
1-7 `137.0s / 41 杀 / 0 漏 / 60,750`；SR-6 三档 `196.6 / 201.4 / 196.6s`
（21 杀 0 漏剩 3 命、617,000）；SR-EX-8 四人剑气 `219.8s / 38 杀 / 1 漏 / 剩 2 命`。
自检 `tools/check_battle.py` 等十一套（见「快速开始」的复现命令）。

#### 三条基线里的敌人核对

干员是方程组的一边，敌人是另一边。同一套判据（**`battle/` 里有结算才算已建模**）
把三条基线里出场的敌人也过了一遍。复现：`python _proto/enemy_audit.py`
（产物 `out/enemy-audit.md`）。

关联两个源没有外键：gamedata 的 `enemy_id` → `EnemyLibrary.name()` → 归一化名字
匹配 prts.wiki 页名。**已知一处对不上**：`enemy_1029_shdsbr` 在 gamedata 里叫
「持盾刀兵」（`name` 与 `display_name` **都是**这个），而图鉴条目叫**「机动盾兵」**
——以图鉴名为准，别名表在 `_proto/enemy_audit.py::ALIAS`。
...
**1-7** 41 只 / 6 种是干净的：源石虫·α×23、士兵×7、暴徒×4、鸡尾酒投掷者×4、
拾荒者×2、机动盾兵×1，全部只有「数值 + 近战/远程 + 移速 + 阻挡 + 目标价值」，
没有一条落在缺口上。这解释了它为什么能当最稳的回归基线。

**SR-6** 21 只 / 3 种，缺口只有「几点了钟」的异常免疫语义。

**SR-EX-8** 39 只 / 8 种，**6 种有缺口**——这就是这关难的根本原因，
不是练度不够，是模型不够：

| 敌人 | 只数 | 缺的机制 |
|---|---:|---|
| “死志的凝结”（BOSS） | 1 | 多形态切换、重生/复活、蓄力、无敌、反弹、阻挡特殊规则、异常免疫、减速 |
| 咆哮铳 | 2 | 反弹（【炸膛】）、减速/停顿 |
| 吓人路灯 | 2 | 隐匿（迷彩）、异常免疫 |
| 蹒跚羽兽 | 10 | 阻挡特殊规则（近地悬浮）、异常免疫 |
| 没办法车 | 1 | 阻挡特殊规则 |
| 挥铳圣像 | 1 | **飞行（不可阻挡）**——这是全表唯一 `is_flying=True` 的 |

⚠️ **一条必须记住的误报**：编译层对「鸡尾酒投掷者」「蹒跚羽兽」「吓人路灯」
都吐过 `fly` 项，但三者的 `is_flying` 全是 **False**。原因是正文措辞——
鸡尾酒投掷者的天赋写的是「**不会攻击**飞行单位」，蹒跚羽兽写的是「近地悬浮」。
**「正文提到了这个机制」不等于「这个敌人有这个机制」**，凡有字段可核的机制
都必须回字段核一遍（`_proto/enemy_audit.py::FIELD_CHECK` 就是这么加的）。
真正的飞行缺口只有挥铳圣像一个，且它由字段坐实（`is_flying=True` + 正文「·飞行单位」）。
...
一场 1-7 要跑 4111 帧，`_update_blocking` / `_pick_targets` / `_resolve` /
`_enemies_attack` 这些每帧函数每次都要扫 41 个敌人，而 `e.alive` 是 property、
`can_block` / `is_at` 是小方法。剖面显示**光是 `alive` 一场就被调 57.5 万次**
（property 访问约 80 ns，平属性读约 18 ns）——派发开销比它做的事还贵。

做法是**就地展开**：把 `e.alive` 写成 `e.hp > 0`、`op.can_block(e)` 写成
内联的阻挡位判断、`is_at` 改成平方比较省掉开方。

| | min | 中位 |
|---|---|---|
| 展开前 | 269.9 ms | 279.5 ms |
| 展开后 | **228.9 ms** | **234.4 ms** |

**1.18×（min）/ 1.19×（中位）**，1-7 基线 137.0s / 41 杀 / 0 漏 / 60,750 **逐字未变**，
十一套自检全绿。

⚠️ **不要改成「摊平成列表再写回」**。那是更慢的做法，实测如下（`_update_blocking` 单函数）：

| 做法 | 本函数 µs/帧 | 端到端 |
|---|---|---|
| 原版 | 14.53 | 1.000× |
| 只摊平（每帧建 7 个列表的搬运底价） | 9.89 | 0.840× |
| 摊平 → 扁平列表上算 → 写回对象 | 20.8（9.3 搬运 + 11.6 算法） | 0.873× |
| **就地展开（不搬数据）** | **8.61** | **1.091×** |

收益全部来自**减少间接层**，与数据放在对象里还是列表里无关。复现：
`python _proto/flat_probe.py`。

⚠️ 另一条实测教训：`point_at` 占剖面 0.038 s，看着"该省掉 `zip(points, points[1:])`
