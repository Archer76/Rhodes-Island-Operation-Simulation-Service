# ak-tactic —— 自动计算明日方舟关卡打法

目标：给定一张关卡地图与一份可用干员名单，让程序自己算出「谁放在哪、什么时候开技能」。

这不是一个小工程。它至少要跨过四道坎：**把数据拿全 → 把数值算对 → 把时间轴模拟出来 → 在巨大的选择空间里搜出可行解**。
所以先把任务拆开，一个一个咬。目前完成的是第一、二、三块：**干员数据（prts.wiki）＋ 敌人与关卡数据（游戏本体 gamedata）**。

数据有两条来源，分得很清楚：**文字资料查 PRTS，机器要算的数值查 gamedata**。
...
python -m ak_tactic enemydb build     # 敌人库，要联网，首次约 53s（PRTS 限速 1.2s/请求）
