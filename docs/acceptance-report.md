...
- 生成时间：2026-09-19 22:28:28
- 执行者：验收与守卫会话 `session-1a45cfee-9a65-4830-a327-03ac84285bfb`（命令：`python tools/acceptance.py`）
- 判定：**通过 7 / 失败 0 / 未跑 1**，退出码 **0**，总耗时 2.9 分钟
- 测量对象：HEAD `475424b78fb9`，脏文件 50 条

> ⚠ **测量期间工作树被改动**：HEAD ee836addd455 → 475424b78fb9；脏文件清单变化（48 → 50 条） ⇒ 本轮数字不对应单一版本，跨轮次比数量之前必须重跑。

## 一、逐项实测

| # | 项 | 状态 | 本轮实测 | 上次基线 | 判据 |
|---|---|---|---|---|---|
| 1 | 仪器自检（Go 二进制） | 通过 | 私有构建 `rios-sim-93352b8f.exe`（`64270de892b32b84`）；共享 exe 落后源码=True | — | 当前源码能构建出 Go 引擎二进制（-buildvcs=false） |
...
- 私有构建（本门使用）：`rios-sim-93352b8f.exe` sha256 `64270de892b32b84`，3,597,312 字节
