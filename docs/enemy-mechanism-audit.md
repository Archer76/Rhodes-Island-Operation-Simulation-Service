...
### 2.3 逐章**单独**跑一遍（每章一次独立调用，2026-09-24 复核）

**为什么另跑一遍**：「第 0～10 章一起跑 141 关」与「每章单独跑自己的那几关」是
**两个口径**——一起跑时，一只敌人只算在**它第一次出现的那一章**（全局首次遭遇归属），
单独跑时每章各算自己引用的全部。两个数**不是同一个数的两种写法**，所以**并排印、
标明口径**，不许互相代替。

**命令**（逐章原样，11 次独立调用；关卡 id 从 `data/gamedata/_level_index.json` 里
按 `^main_NN-\d+$` 现取，**不是手抄**）：

```powershell
# 每章取自己的那一批 base 关卡 id，再喂给判据（一章一次调用）
$l = (python -X utf8 -c "import json,re;idx=json.load(open('data/gamedata/_level_index.json',encoding='utf-8'));print(' '.join(sorted(k for k in idx if re.match(r'^main_00-\d+$',k))))") -split ' '
python -X utf8 tools\check_enemy_mech_go.py @l      # 对 "00".."10" 各做一次
```

⚠ 两个**必须**做的事（否则读到的是假读数）：
① 关卡 id 要**拆成 argv 数组**（`-split ' '`）——整串传过去会被判据当成**一个**关卡 id，
   `MECH-CRASH InvalidURL … control characters` 并以 `rc=5` 退出（本轮的第一次就是这样）；
② 别拿 `$LASTEXITCODE` 读管道的 rc（PowerShell 的管道会吃掉它）——判据自己的 rc 要
   让它自己印出来，或写成文件再读。

**读数**（11 次全部 `rc=0`；`enemies`／`keys` 是**章内去重**）：

| 章 | 关数 | 敌人 | 机制键 | 键次 | 已消费 | 具名 unported | **真红** | 共同边界 | 仅 Go | 正文无键 | 正文判不了 | 风味 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| main_00 | 11 | 8 | 0 | 0 | 0 | 0 | **0** | 0 | 0 | 1 | 0 | 9 |
| main_01 | 12 | 15 | 3 | 3 | 0 | 0 | **0** | 3 | 0 | 1 | 4 | 18 |
| main_02 | 10 | 26 | 5 | 5 | 0 | 0 | **0** | 5 | 0 | 4 | 3 | 36 |
| main_03 | 8 | 31 | 5 | 5 | 0 | 0 | **0** | 5 | 0 | 8 | 13 | 34 |
| main_04 | 10 | 25 | 12 | 12 | 0 | 0 | **0** | 12 | 0 | 5 | 6 | 29 |
| main_05 | 10 | 24 | 8 | 9 | 0 | 0 | **0** | 8 | 0 | 7 | 13 | 26 |
| main_06 | 15 | 28 | 25 | 37 | 0 | 0 | **0** | 25 | 0 | 5 | 38 | 24 |
| main_07 | 16 | 28 | 18 | 31 | 0 | 0 | **0** | 18 | 0 | 8 | 19 | 46 |
| main_08 | 17 | 25 | 79 | 114 | 0 | 0 | **0** | 79 | 0 | 11 | 27 | 28 |
| main_09 | 17 | 13 | 38 | 51 | 0 | 0 | **0** | 38 | 0 | 0 | 27 | 11 |
| main_10 | 15 | 22 | 47 | 78 | 1 | 0 | **0** | 46 | 0 | 0 | 52 | 14 |
| **合计** | **141** | 245 | 240 | 345 | 1 | **0** | **0** | 239 | 0 | 50 | 202 | 275 |

**两个口径的数**（★ 跨行不可比）：

| 口径 | 机制键 | 共同边界 | 正文判不了 | 说明 |
|---|---:|---:|---:|---|
| 141 关**一起**跑（§二 那张表） | 229 | 228 | 179 | 每只敌人只算在**首次出现**的那一章 |
| **每章单独**跑（本表） | 240 | 239 | 202 | 每章各算自己引用的全部 |

**逐章结论**（这就是「0～10 章有没有未建模机制」的答案）：

1. **`真红` 11 章全为 0**：`真红` 的定义是「Python 有落点、Go 没有、且未登记」
   ——**Go 在主线 0～10 章的敌人机制上一处未登记欠账都没有**（141 关、每章单独跑也成立）。
2. **`具名 unported` 11 章也全为 0**：第 27 套登记表那 11 个键的载体全在**活动侧**
   （怀黍离田鼷／祟／天桩一族），主线一条都碰不到。★ 这**不是**「都搬完了」，
   是**这批敌人的键根本没进过那张表**。
3. **但「没有未建模机制」不成立**：每章都有一批 `共同边界` 键（合计 239）——
   那是**两台引擎都没有落点**的机制（`shield.*`／`strength.*`／`aura.*`／`halfhp.*`／
   `refracting.*`／`atkup.*`／`deathrattle.*` …）。**两侧都没建，所以不算 Go 的欠账，
   但它确实没被建模**。逐条列在第五节。
4. **还有 202 条「正文判不了」**：正文／图鉴里写着的机制，**没有具名判据**能判它建没建
   ——这是**未核**，不是 0。逐条列在第六节。
5. **`已消费` 全章合计只有 1**（main_10 曼弗雷德的 `Reborn.interval`，`sim.go:828` 有落点）
   ——它不是「主线只有 1 个被消费」，而是**主线敌人的键绝大多数落在共同边界上**。

### 2.4 非敌人机制层（田地／雪／装置／符文）在 0～10 章的逐章状态

**数据来源＝冻结基线本身**（`fixtures/golden/机制规格.json` 里 `["mechspec", 关卡, sha16]`
那 562 条的值，就是**权威侧（Python）**对每一关算出来的 `mechanisms` ＋ `mech_config`）。
只读一个 JSON 文件、不跑任何引擎 ⇒ 这是**权威侧（Python）说主线有没有这些机制**，不是 Go 说。

| 章 | 关卡键 | 有田地 | 有雪 | 装置条数 | 机制名 |
|---|---:|---:|---:|---:|---|
| main_00 ～ main_17（**逐章全是 0**） | 22/22/20/16/20/20/28/32/33/34/15/18/18/19/20/34/32/31 | **0** | **0** | **0** | **（空）** |

**★ 正对照**（免得把「探针恒 0」当成结论）：同一份基线里 **562 个 mechspec 键中有 26 个非空**，
全部落在 `act31side_*`（怀黍离），键里带 `huai_shu_li.farmland`。
⇒ 探针**不是哑尺子**：它能看见非空，而主线**确实全空**。

**结论（逐章）**：

1. **第 0～10 章的「机制层」一个都没有**：`mechanisms` 空、`mech_config` 空
   ⇒ 没有田地、没有雪、没有装置。**11 章逐章如此，不是「合计为 0」**。
2. 机制层整族住在**活动侧**（怀黍离 `act31side` 26 关有田地）。
   所以「主线有没有未建模的机制层」这一问的答案是：**没有可建模的对象**，
   不是「建了」也不是「漏了」。
3. **装置 id 那一面另有独立取证**：把主线 0～10 章的关卡文件里
   `predefines` / `hardPredefines` 的条目全取出来逐个看，**一个 `trap_*` / `token_*` 都没有**
   （出现的只有 `tokenInsts`／`tokenCards`／`characterInsts`／`characterCards` —— 那是
   **预置干员/召唤物**，不是装置）。⇒ 与上一条独立地指向同一个结论。
4. **符文／关卡选项（`options`／`runes`：属性倍率、环境系统、生命点、初始费用…）是另一层，
   不在本判据的取证范围里** —— 它们由 **`关卡静态`（`check_stageenv_go.py`，`stage_env`）**
   那一套管（已转、已绿，见 `golden-baseline.md` 第 13 行）。
   ⚠ **本文件不为它背书**：我**没有**逐章核对「Go 消费了 options 里的哪些键」——
   这是**未核**，不是 0。要闭合它，办法是把 `关卡静态` 的键按章分组再读一次。
