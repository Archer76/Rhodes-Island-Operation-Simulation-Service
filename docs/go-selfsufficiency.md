...
| 判据 | 取证面 | 读数 |
|---|---|---|
| 关卡 `check_stage_go.py` | 缓存可达的 **562 个关卡键**（286 份不同内容） | **562 / 562 关逐字段一致** |
| 敌人 `check_enemy_go.py` | 那 562 个关卡键引用的全部敌人 | **3077 只敌人逐字段一致** |
...
| 关卡静态 `check_stageenv_go.py` | 缓存可达的 **562 个关卡键**（286 份不同内容）× 难度标签 ＋ 8 例合成 | **1140 例逐字段一致**（另 0 例两边都拒、0 处失配） |
| 格表 `check_cells_go.py` | 缓存可达的 **562 个关卡键** | **562 关逐格一致**（另 0 例两边都拒） |
| 部分规格 `check_specgo_go.py` | **562 个关卡键** × 12 个值键 ＋ 键集账（ast 抽 19 键）＋ 24 份夹具 × 生产规格 ＋ **deploys 64 条** ＋ **6 份合成计划的 skill_uses 18 条**（故意乱序） | 562 关的 12 个值键逐项一致，键集账 19 个键对得上 |
| 费用天赋 `check_costbonus_go.py` | 12 种黑板形状 × 10 种队伍组合 | 10 / 10 次求解一致 |
| 部署费用 `check_costof_go.py` | 24 份夹具 × **生产规格的 `deploys[].cost`** | 64 人次一致 |
| 寻路 `check_stagepath_go.py` | **562 个关卡键**的全部路线 × 两档斜向 ＋ **全部路线的分段计划**（另 7 条合成路线）＋ **全部路线的计划表**（`eta.route_plans`） | **32736 例逐格一致**；分段 **33650 段**逐字段一致（`length` **逐位**）＋ 合成 14 段；计划表 **16368 条路线 33650 段**逐字段一致（`points`/`wait`/`length`/`seconds` 全精确） |
...
| 机制规格 `check_mechspec_go.py` | 缓存可达的 **562 个关卡键**（**空排程口径**）＋ 24 份夹具的**生产口径**对账 | **562 关逐字段一致（有田地 26 关 / 无田地 536 关）**；生产口径 24 份：田地逐字段相同 24 份、`mechanisms` 差恰为雪；未搬 1 条；天桩召唤链逐字段比过 35 条（6615 个字段） |
...
| 单一入口 `check_buildspec_go.py` | **三个口径**：A 计划（24 份夹具的生产规格）＋ B 空计划（缓存 **562 个关卡键**）＋ C 计划侧合成 3 例（把真夹具零行使的 `unsupported`／`skill_uses` 走到） | **580 例逐路径一致（A 24 ＋ B 553 ＋ C 3）**（19 键全比）；带雪 2 例已逐字段比；已登记缺键 **0** 处（`devices[].child` 与 `operators[].shield` 都已搬进 Go）；`missing_keys`／`gated_keys` 全空；B 另有 **9 关具名拒跑**（未实现的敌人修饰层，不计入可比分母） |
| 自造规格 `check_sim_selfspec_go.py` | **25 份夹具**（24 真 ＋ 1 合成），**差分口径**：同一关同一计划下「查询形式」与「先 `buildspec` 再送规格」各跑一遍 | **25 例差分一致（判决 24 例逐路径相同、拒跑 1 例同因、`unsupported` 25 例逐位相同）**；契约组 5 条具名失败全成立；三种结局（判决／拒跑／`unsupported`）都被真行使；口径控制组：`allow_devices` 回显跟着翻而结局不变 |
...
| 敌方机制 `check_enemy_mech_go.py` | **两条腿、现算两面求差**：数据侧＝缓存可达的 **562 个关卡键**（＝286 份不同内容，见 check_go_all 那一行的口径）引用的全部敌人（天赋黑板 ＋ 敌方技能黑板的键，另有 gamedata 正文／prts `talent`／`ability`）；Go 侧＝`rios-sim/*.go` 的读取点（36 个键的字面读取点 ＋ 重生族／相性族两条具名族规则）＋ 行为落点；**Python 侧＝`ak_tactic/gamedata/enemy.py` 的字面读取点 ＋ `ak_tactic/battle/*.py` 的行为落点**。**不跑二进制** | 去重键按章累计：**已消费 41 ／具名 unported 17（11 个键，逐条带出处）／真红 0 ／共同边界 238 ／仅 Go 0**；正文已登记 20、正文无键 32、判不了 194（均不计入 rc）。★ **登记 ≠ 已修复**：`NAMED_UNPORTED_KEYS` 那 11 条是「Go 读了黑板、填了字段、模拟里没人读」，成立与否**每次现算**（读取点／字段名／字段是否已有行为落点），过期即印 `MECH-STALE-REGISTRY` 并退回真红。反向守卫四组腿（Go 两方向 ＋ Python 腿 ＋ 边界守卫 ＋ **登记保活**）共 11 项全成立。★ 第 0～10 章**逐章单独**一跑（11 次独立调用，全部 rc=0）：真红 0、具名 unported 0、共同边界 239（章内各自口径）／228（141 关一起跑口径）—— 主线的敌人机制整族落在**两台引擎共同的边界**上，逐章读数见 [`enemy-mechanism-audit.md`](enemy-mechanism-audit.md) §2.3 |
...
5. **「读数」不稳定 ≠ 「判据」不稳定——但两者必须分得开，且「抹平了什么」要写出来。**
   实测（2026-09-24，`机制规格`）：那一套印一个计数器「`groups 顺序不同`」，
   **同一份未改代码连跑两次得 `17` 与 `13`**，历史上见过 `13／14／16／17` 四个值
   （Go 侧 `map` 迭代顺序每进程一次随机，Python 侧那条是 `set.pop()`）。
   它**不进判据**（判据按**格集合**口径比、两侧各自排序），所以**判决稳定**；
   但它意味着这一套的「默认档改前＝改后**逐字相同**」**不可能逐字节满足**——
   只能把那一行**抹平后**再比（作者这次就是这么做的：`IDENTICAL-MASKED（42 行）`）。
   ⇒ 两条附注：**冻结档也治不了它**（Go 那一侧仍是活进程）；
   **「抹平了哪一行」必须写出来**，否则下一个人会以为自己比的是逐字节。
...
| **待重录** ◑ | **已经接好了**，只是「录的是哪一批对象」变了（缓存长大、夹具改了）⇒ `rc=6` ＋ 印「输入批次对账」 | ❌ |
| **待转** | 还没接（`rc=6`，具名理由：「从未绑定通道」） | ❌ |
| **不适用于冻结** | 结构上没有「Python 侧的期望值」可冻 | ❌ |

**分子是第一个，分母是五者之和。** 把「部分覆盖」算进分子，主判据就会**虚高**——
这正是本仓那条「**分母静默缩水**」的**镜像**（这次缩的是分子）。

★ **`待重录` 与 `待转` 必须分开**（2026-09-24 实测逮到）：两者都 `rc=6`，但前者
**已经做完迁移**、只差一次 `--record`；压成一个「待转」标签，读的人会去重做一套做完的活
（实测：缓存从 320 键长到 562 键后，`格表` 被印成「待转」，而它只是对象集变了）。
`cmd_status` 现在按**输出里有没有「输入批次对账」**分这两态，`◑` 与 `✗` 是两种标记；
`named_reason()` 也改成**优先取那一行**——否则印出来的「理由」会是通道自己的 summary
（「取期望值 320 次全部命中…」，那句是真的，但它是**结论**不是**因**）。
