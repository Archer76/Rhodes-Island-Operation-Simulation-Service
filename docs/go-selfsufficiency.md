...
* ⚠ **第三个数**：那 11 份原先被拒跑**挡在判决之外**，键一接上才看得见其中 2 份
  （`hsex8_max` 48→19 杀、`plan-hs07` 96→25 杀）本就因**另一个键**而分歧——
  `mech_config["snow.field"]`（雪）。构造法证死：把它挖掉即**逐位复现**现读数，
  而对齐 `goal_cells` 无效。雪要**排程**（`snow_mech_spec` → 无垠的雪景 ＋ 部署时间线），
  那是第三条线的活；且 `mechspec` 这条命令**按契约不吃计划**（判据里有这一条）。
  ⇒ 处置：判据里**登记 ＋ 计数 ＋ 当场复现 ＋ 反证**，份数印在结论行上，**待裁定**。
