...
**取数四层（关卡／敌人／干员／名册）已经搬进 Go，且每一层都有跨实现对拍判据。**
缺的是「把效果折成两套数值」的**装配**与「规格构造」——那两段还在 Python。
两套数值要的那几处读数已全部落地为纯函数（`rios-sim/profile.go`、
`panelfold.go`）；没落地的是**喂它们的实时输入**（光环／翔虫机动／替身计时／
击杀叠层／出手次数／阻挡／高台邻居这些运行态），以及**把名册的练度接到那些
读数上**——名册读进来了，但还没有消费点。
...
* 前半：**十套判据现在不报红**；
* 后半：**十套的反向守卫都成立**（每套人为注入一处不一致，它真会红）。
...
| **名册 → 规格的消费** | 名册已经读进来了（`rios-sim/roster.go`），但**还没有消费点**：精英／等级／潜能／模组要接到面板那几处读数上，才谈得上「Go 自己构造规格」。 |
| **计划（打法）的读取** | `ak_tactic/plan.py` 的 `Plan.from_dict`（部署／撤退／技能三条指令序列）在 Go 侧同样是零入口。目标里点名的「＋计划」这一层尚未动。 |
