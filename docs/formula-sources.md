...
| 信赖 | `favorKeyFrames` | `level` 是 **0–50**，对应**显示信赖 0%–100%**——**显示到 100% 即封顶**，再往上（游戏可显示到 200%）只解锁资料与语音、**不加属性**。故 `level = 显示信赖 ÷ 2`，而 `stats(trust=)` 收的**就是这个 level**（名册的 `favorPercent ÷ 2` 同尺度），**不要再除一次**。⚠️ 2026-09-17 更正：旧写法按「level 0–50 对应显示 0%–200%」又除了一次，依据是把**助战干员**的旧换算（「信赖值除以 2 再向下取整」，2025/12/5 已取消）当成了通用规则；症状是 154% 信赖只给 +46.2 攻击而非满值 +60 |
...
* 怒潮凛冬 精2 60 潜1 信赖200% + 模组 `uniequip_002_headb2` Lv1
  = **2981 / 1307 / 473**（2026-09-17 博士实机）——三个数一起把**取整**钉死：
  攻击一栏 `floor(1193.91 + 50 + 63)` 给 1306，只有 `round` 给 1307。
  同一人**零信赖、无模组**时 = **2731 / 1194 / 387 / 0 / 攻击间隔 1.80 / 阻挡 2 / 费用 20**，
  落在**插值中段**，验的是关键帧与插值口径（旧记录写 1193，是 floor 口径下的读数）。
