...
## 〇、⚠ 可复现性（2026-09-20 01:45 补记，PM 派活账二）

**本文以下所有"72 关"「交集 80 关」的读数，当前不可复核**——比对的输入是
`data/gamedata/<三个镜像>/level_*.json` 的 **`GameDataSource` 按需缓存**（**随用随下、不入库、不进报告**），
而**该缓存已被重建**。本回合（2026-09-20 01:40）实测：

| | 当时（2026-09-19 23:19） | 本轮实测（2026-09-20 01:40） |
| --- | --- | --- |
| 本地缓存 | 105 份 → 去重 **83 关** | **22 份 → 22 关** |
| 与第三源交集 | **80 关** | **22 关** |
| 实比 | **72 关**：完全一致 72、分歧 0、加载失败 8 | **22 关：完全一致 22、分歧 0、加载失败 0** |

⇒ 按既定纪律写：**结论成立，但当前不可复现**——**不是"当时错了"**，也**不许**追认成"当时记错了"。
**缺什么**：那 72 关对应的缓存 JSON（尤其 `act31side_*` 之外的关卡）。

**为什么不能入库**：① `data/` 目录**零入库**是既定法规（经理侧合规自查亦以此为据）；
② 地图层数据属第三方派生数据，**分发红线**（见 `docs/` 合规结论）⇒ **缓存不能进仓**。
**怎么恢复可复核**：跑一遍那 72 关（让 `GameDataSource` 把缓存下回来）后重跑本工具；或对照下面的锚点判断缓存是否已变。
**本轮锚点（22 关的关名，仅名字、不含数据）**：`a001_01`、`act31side_01/02/03/04/05/06/07/09`、
`act31side_ex01..ex08`、`act31side_tr01/tr02`、`act54side_06`、`act54side_ex08`、`main_01-07`。
...
- 两边都是 `tiles[y][x]`（原点左上、y 向下），且**同概念**对齐（都取 `tileKey == tile_start/tile_end`）。实测 **72/72** 关的起点格完全一致 ⇒ **同向，未翻转**。（**当时读数，当前不可复核**，见 §〇。）
- **没有一关**起点格不一致。
...
（**当时读数，当前不可复核**；本轮实测为 22 关、完全一致 22、0 分歧、0 失败——见 §〇。）
