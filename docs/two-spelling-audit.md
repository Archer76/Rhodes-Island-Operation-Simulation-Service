...
## 二、A 档逐条（本档 0 条）——机械规则先报的 7 个已**逐条推翻**，各自归入 B（同值）／E（口径·后缀读法）／D（无干员行），证据在对应节里
...
|---|---|---|---|---|---|
| `prob` | `$prob` | 4/0 | 1 | 触发概率：命中/触发次数；**方向须一次对照实验**　※ 没人读的是 `$`（字符串）侧 ⇒ 接的是那一侧的值 | `$prob` 与裸 `attack@prob`/`prob` **同值**（实测 4 处同值、0 处不同值）；而且 `$prob` 只出现在**装置**行，干员侧（Misery）根本没有 `$` 侧 ⇒ 假欠账 |
...
* **`$` 侧 vs 裸侧**：同值 17 / 不同值 0
* **可见量与方向**：召唤物/子单位 id　——　该概念的值是**字符串**（另一个量）⇒ 接上后 召唤物/子单位 id按该字符串设定；方向须与正文比对
...
|---|---|---|---|---|---|---|
| `projectile_range` | **有** | `$attack@projectile_range`、`$projectile_range`、`attack@projectile_range` | 50 | 引星棘刺｜char_1039_thorn2、机械师｜char_4230_mcnist、艾拉｜char_4123_ela | 弹道射程（Go 侧弹道机制）：**须一次对照实验**（机制在 `rios-sim/` 侧）　※ 没人读的是 `$`（字符串）侧 ⇒ 接的是那一侧的值 | 4 位干员（引星棘刺／机械师／艾拉…）、50 行；本树**没有任何** `projectile_range` 读点 ⇒ 要新读点；弹道机制在 `rios-sim/` 侧（机制层） |
...
| `attack_range_id` | **有** | `$attack@attack_range_id`、`attack@attack_range_id` | 10 | 予愿安洁莉娜｜char_1015_aglna2 | 规格 range 格表：该概念的值是**字符串**（另一个量）⇒ 接上后 规格 range 格表按该字符串设定；方向须与正文比对 | 予愿安洁莉娜 1 位、10 行；**已按判据③判红**：`x-4` ＝ 周围 8 格、接进 `_range_override` 会让范围 19 格 → 9 格，与正文「扩大」**方向相反** ⇒ **不许接**（`docs/uncertainties.md` 第三十节） |
