...
| `eng/tools/rebuild_data.py` ＋ `fetch_prts_notes.py` | 首次运行第 3 步要跑它（§12.5）⇒ 属于运行时必须 |
| `启动.cmd` | 一键入口，且**启动器自检**（§12.3）落在它身上 |

★ 打包脚本必须具名登记一件容易漏的事：`tools/fetch_prts_notes.py` **在产品仓 `.gitignore`
的「内部件」名单里**（它被摘出过），而运行时要用它 ⇒ 打包时它只能从**本机工作树**取。
不登记的话，打包脚本会「照 `git ls-files` 找文件、找不到就静默少一件」，
装出来的包在首次运行第 3 步才炸 —— 那正是 §12.3 要防的形状。
...
`AK-TACTIC-进度.md` 这类**文件名**、`..\ak-tactic-worklog` 这类**相对写法**
逐条登记（每条给理由），其余必须为 0。为什么不能追求字面 0：判据 6 同时要求
**树哈希与改前一字不变**，而 `..\ak-tactic-worklog` 这类串在 blob 里本来就有
（是上一轮改写**故意**换成的相对写法）—— 要字面 0 就得改 blob，那就违反"树不变"。
**绝对路径一律洗**（`C:\Python314\python.exe` 那三处本轮已洗），名字与相对写法登记保留。
