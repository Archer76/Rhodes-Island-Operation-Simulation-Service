...
1. **`ep_damage_ratio` 乘的是什么 ⇒ 已取证：乘的是 ATK。**
   ...
   仓库现有规则把三种写法**都**记成 `source="ATK"` —— 这是**可能的口径合并**。
   **取证原版的结果（通告 #7 §一 第一步，已完成）：原版根本没有结算层可读。**
   ...
   `ak_tactic/battle/`（本次门的参照物）里 `ep_damage` / `损伤` **零命中**；
   唯一沾边的是 `p3r.py` 的 `TotalAttack.ELEMENT`，而公式层自己写明「这与 P3R 的**敌方元素相性**是两条不同的轴，别混」。
   ⇒ **结论：元素损伤没有「原版结算」可对照**，所以「逐位一致」这条判据对它**不适用**，
   「伤害X% vs 攻击力X%」**不能**靠取证原版解决 ⇒ 只能进**待博士表**（`docs/uncertainties.md`）。
   ⚠ **该表的落点要注意**：`docs/uncertainties.md` 是 `python tools/uncertainty_audit.py`
   **生成**的，文件里明写「表中其余各栏都是实测数据，请不要改（改了下次生成就没了）」——
   所以要进表**必须在生成器的题源里加**，不能手写一行。**此事归谁动，已广播请裁。**
   ⇒ **对窗口的影响：无。** B 组"黑板原样映射"只搬键值、不解释乘数基，
   **与这个歧义正交**；A 组标量形式同理。等博士裁了再定乘数基。
